...
from Interfaces.VentPrinc_Alumno_interfaz import *
...
from PySide6.QtWidgets import QPushButton, QApplication, QLineEdit, QLabel, QDialog, QMainWindow
...
class VentAlum(QMainWindow):
    def __init__(self, CodUser, RolUser, NomUser, ApellUser, CorreoUser):
        super(VentAlum, self).__init__()
        self.ui = Ui_MainWindow()
        self.ui.setupUi(self)
        self.show()

    # Esto oculta la barra de título
    ...
    #esconder boton inecesario
        self.ui.Bt_MasOpciones.hide()
        self.ui.Bt_Regresar.hide()
        self.ui.Bt_CargarChats.hide()
        self.ui.Bt_RecargarGrupos.hide()
    ...
    # Botones para cambiar entrdde los ajustes 
    ...
    # Unirse al Grupo
        self.ui.Bt_BuscarGrupo.clicked.connect(self.UnirseG)
...
# Boton Unirse al Grupo
    def UnirseG(self):
        self.vent = BuscarGrup(self.CodUser, self.NomUser, self.ApellUser)

# Cambiar entre las interfaces de los Chats y Grupos
...
# Cambiar entre los chats       
    ...
    def crear_burbuja_mensaje(self, mensaje, tipo, emisor=None, fecha=None):
        ...
        if self.MenGrup == 1 and tipo == "recibido" and emisor and fecha:
            ...
            L_emisor.setStyleSheet("font-weight: bold; font-size: 11px; color: #1A73E8;")  # Azul Google
...
# Funcion de Enviar Mensajes
    ...
    def cargar_mensajes(self, ID_Chat):
        while self.ui.verticalLayout_9.count() > 0:  # Deja el último spacer
...
#Obtiene los nombres de los 
    ...
    def CamCont(self):
        ...
        stored_password = self.CambiarCon.BuscarCont_Alum(ContAct)
        if stored_password is None:
            ...
            if len(NCont) < 8 or len(NCont) > 16:
                ...
                # Aquí puedes actualizar la contraseña en la base de datos
                    ...
                    self.CambiarCon.ActualizarContAlum(NCont, self.CodUser)
...
class BuscarGrup(QMainWindow):
    ...
    def __init__(self, CodUser, NomUser, ApellUser):
        ...
        self.ui.StackBuscar.setCurrentIndex(1)
        ...
        self.ui.Bt_BuscarGr.clicked.connect(self.MandarDatos)

        self.crear = ManejoDeGrupos()
        self.VenPr = VentAlum(CodUser, RolUser, NomUser, ApellUser, CorreoUser)

        self.dialogo.exec()

    def MandarDatos(self):
        CodG = self.ui.LE_CodigoBuscarG.text().strip()
        if CodG == "":
            messagebox.showwarning(title="ERROR", message="No has ingresado un código")
        else:
            integ = self.NomUser + " " + self.ApellUser
            CodInteg = self.CodUserX

            Ver = self.crear.UnirseGrupo(CodInteg, integ, CodG)
            self.crear.ActualizarAlumno(CodInteg, CodG, Ver)
...
if __name__ == "__main__":
    ...
    CodUser = 123456789
    RolUser = "ValorRolUser"
    NomUser = "ValorNomUser"
    ApellUser = "ValorApellUser"
    CorreoUser = "ValorCorreoUser"

    app = QApplication(sys.argv)
    mi_app = VentAlum(CodUser, RolUser, NomUser, ApellUser, CorreoUser)   
    mi_app.show()
    sys.exit(app.exec())